...
// only Bc - F Ne
const Bc2FNe = (document.getElementById("periodic-2stR")) ? true : false;
document.getElementById("periodic-2stR")?.appendChild(tb);
...
if (HDTHe) {
    ...
    for (const line of "H氕\tD氘\tT氚\tHe氦".split("\n").map(e => e.split("\t"))) {
        ...
        for (const elem of line) {
            ...
            const translation = englishName[elem.slice(0, -1).replace(/[\u4e00-\uFfa5]/g, "")] ?? "?";
            td.innerHTML = `<div class='id-num'>${id_num}</div><span class="elem-name">${elem}</span><div class='en-name ${translation.length > 9 ? "smaller" : translation.length > 7 ? "small" : ""}'>${translation}</div><div class='electron ${textLen > 3 ? "smaller" : textLen > 2 ? "small" : ""}'>${electronsText[id_num - 1].join("")}</div>`;
            id_num++;
        }
    }
} else if (Bc2FNe) {
    tb.style.width = "375px";
    id_num = 9;
    for (const line of "Bc磐	B硼	Qb矻	Q磻	Qc砠	C碳	Tk𥓬	E硝	Tn磴	N氮	Ny氤	O氧	Fn氛	Fd氲	F氟\tNe氖".split("\n").map(e => e.split("\t"))) {
        const tr = document.createElement("tr");
        tb.appendChild(tr);
        for (const elem of line) {
            const td = document.createElement("td");
            tr.appendChild(td);
            td.classList.add(id_num === 24 ? "Hepleto" : "nonmetal");
            if (elem.length === 1 || !elem.trim()) { td.innerHTML = "<br/>"; td.classList.add("empty"); continue; }
            const textLen = electronsText[id_num - 1].length;
            const translation = englishName[elem.slice(0, -1).replace(/[\u4e00-\uFfa5]/g, "")] ?? "?";
            td.innerHTML = `<div class='id-num'>${id_num}</div><span class="elem-name">${elem}</span><div class='en-name ${translation.length > 9 ? "smaller" : translation.length > 7 ? "small" : ""}'>${translation}</div><div class='electron ${textLen > 3 ? "smaller" : textLen > 2 ? "small" : ""}'>${electronsText[id_num - 1].join("")}</div>`;
