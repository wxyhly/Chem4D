...
function bracheClone(a) {
    let b = { name: a.name, nexts: a.nexts.map(bracheClone), bonds: [] };
    if (a.charge) {
        b.charge = a.charge;
    }
    if (a.prebond)
...
export class Parser {
    ...
    nextAtom() {
        ...
        let atom = { name: this.getChar(), nexts: [], bonds: [] };
        let c = this.getChar();
        if (c === "*") {
            atom.name = "";
            while (c === "*") {
...
class GraphBuilder {
    ...
    sssr() {
        ...
        for (let rid = 0; rid < this.rings.length; rid++) {
            let bridgesCount = 0;
            let nextRing = Infinity;
            for (let a of this.rings[rid]) {
                if (!this.ringGroup[rid])
                    continue;
