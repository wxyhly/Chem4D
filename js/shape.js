import { Checker4D } from "./checker.js";
export class ShapeBuilder {
    ...
    drawRingArc(ring, swap) {
        let first = NaN;
        let last = NaN;
        for (let i = 0; i < ring.length; i++) {
            if (!ring[i].position && ring[(i + ring.length - 1) % ring.length].position) {
                first = (i + ring.length - 1) % ring.length;
            }
            if (ring[i].position && !ring[(i + ring.length - 1) % ring.length].position) {
                last = i;
            }
    ...
    drawBranch(self) {
        ...
        for (const b of self.bonds) {
            ...
            if (p)
                vecs.push(Math.atan2(p.y - self.position.y, p.x - self.position.x));
            if (!p)
                finished = false;
        }
        if (finished)
        ...
        if (vecs.length === 0) {
            ...
            for (const b of self.bonds) {
                ...
                if (b.type === 8)
                    radius *= 1.5;
                if (this.isHydrongen(s) || this.isHydrongen(self))
                    radius *= this.hydrogenBondScale;
                if (this.isCluster(s) || this.isCluster(self))
                    radius *= this.hydrogenBondScale;
    ...
    build() {
        ...
        if (g.rings.length) {
            ...
            let start = g.atoms[0];
            while (start.prevatom)
                start = start.prevatom;
        ...
        while (this.frontier.length) {
            ...
            if (!ring || finisinedRings.has(ring)) {
                this.drawBranch(f);
                count++;
                continue;
            }
