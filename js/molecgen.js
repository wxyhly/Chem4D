...
for (const data of rawdata) {
    ...
    if (n.endsWith("酸")) {
        ...
        if (s.includes("OD") && !s.includes("<-Fd->H") && !n.includes("油酸")) {
            ...
            if (n.includes("次") || n.includes("原") || n.includes("铬") || n.includes("塭"))
                continue;
    ...
    if (n.endsWith("腤")) {
        if (!n.includes("磐"))
            database.unshift([r, s.replace("=>>", "=>").replaceAll("N", "Tn"), n.replace("腤", "滕").replaceAll("N", "Tn")]);
        if (!n.includes("硼") && !n.includes("磐"))
            database.unshift([r, s.replace("=>>", "=").replaceAll("N", "E"), n.replace("腤", "亚胂").replaceAll("N", "E")]);
        if (!n.includes("矻") && !n.includes("硼") && !n.includes("磐"))
            database.unshift([r, s.replace("=>>", "<=").replaceAll("N", "Tk"), n.replace("腤", "亚𥓬胂").replaceAll("N", "Tk")]);
    }
    if (r === 12) {
        // 金属氢化物
        const valent = s.includes("+]") ? 1 : s.includes("+2]") ? 2 : s.includes("+3]") ? 3 : s.includes("+4]") ? 4 : 0;
    ...
    if (s.startsWith("C->C") || s.startsWith("CC") || s.startsWith("C=C") || s.startsWith("C#C") || s.match(/^[C=][C=][C=]/)) {
        ...
        if (!n.startsWith("1-氚"))
            database.unshift([3, "T->" + ss, "1-氚" + nn]);
    ...
    if (s.startsWith("Fd%1") || s.startsWith("Ld%1") || s.startsWith("Fn%1") || (s[1] === "%" && (n.endsWith("咯") || n.endsWith("喃") || n.endsWith("啶") || n.endsWith("唑") || n.endsWith("咱") || n.endsWith("䓬")) && !n.includes("异"))) {
        ...
        if ((!s.startsWith("Fd%1->") && !s.startsWith("Ld%1->") && !s.startsWith("Fn%1->")) || hasD)
            database.unshift([r, s.replace(/C/g, "Tk"), prefixD + "𥓬环" + n.replace(/.氢/, "")]);
        database.unshift([r, s.replace(/C/g, "Qc"), prefixD + "砠环" + n.replace(/.氢/, "")]);
        if (hasD) {
            if ((!s.startsWith("Fd%1->") && !s.startsWith("Ld%1->") && !s.startsWith("Fn%1->")) || hasD > 4)
...
export const mdata = database.filter(e => e[1]).sort((a, b) => (b[0] - a[0] + Math.random() * 0.1 - 0.05));
console.log("molecule database size: ", mdata.length);
//# sourceMappingURL=molecgen.js.map