import { BondType } from "./parser.js";
...
export class CanvasDraw {
    ...
    drawMolecule(atoms: Atom[], bonds: Bond[]) {
        this.calcDim(atoms);
    ...
    drawAtom(a: Atom, id: number) {
        ...
        if (a.name[0] === "*") {
            ...
            for (let i = 0; i < a.name.length; i++)this.ctx.arc(cx, cy, (i + 1) * 10 * scale, 0, Math.PI * 2);
            this.ctx.fill();
        } else {
            this.ctx.fillStyle = (a as any).error ? "red" : a.name in COLOR_TABLE ? COLOR_TABLE[a.name] : "rgb(100, 100, 100)";
            // this.ctx.font = "20px Arial";
            this.ctx.font = (a as any).error ? Math.round(44 * scale) + "px Arial" : Math.round(33 * scale) + "px Arial";
            ...
            if (a.charge) this.ctx.fillText(a.charge === 2 ? "2+" : a.charge === 1 ? "+" : a.charge === -1 ? "-" : a.charge === -2 ? "2-" : a.charge === -3 ? "3-" : a.charge < 0 ? (-a.charge) + "-" : (a.charge + "+"), cx + dim.width / 2, cy - dim.fontBoundingBoxDescent);
        }
        // if ((a as any).oma) this.ctx.fillText((a as any).oma, cx, cy+15);

    }
    drawBond(b: Bond) {
        if (!b.start.position || !b.stop.position || !(b.type >> 4)) return;
        ...
        const drawBondLine = () => {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(
                ax * scaleStart + this.canvas.width / 2,
                -ay * scaleStart + this.canvas.height / 2
            );
            this.ctx.lineTo(
                (bx * (1 - ratio) + ratio * ax) * scaleStop + this.canvas.width / 2,
                -(by * (1 - ratio) + ratio * ay) * scaleStop + this.canvas.height / 2
            );
            this.ctx.stroke();
            ...
            this.ctx.lineTo(
                bx * scaleStop + this.canvas.width / 2,
                -by * scaleStop + this.canvas.height / 2
            );
            this.ctx.stroke();
        }
        drawBondLine();
        ...
        if ((b.type & 0xF) !== 8) {
            ...
            for (let i = 0; i < (b.type & 7); i++, cx += arrowLenX * gap, cy += arrowLenY * gap) {
                this.ctx.moveTo(
                    (cx + arrowLenX + arrowWidY) * scaleStop + this.canvas.width / 2,
                    -(cy + arrowLenY - arrowWidX) * scaleStop + this.canvas.height / 2
                );
                this.ctx.lineTo(
                    (cx) * scaleStop + this.canvas.width / 2,
                    -(cy) * scaleStop + this.canvas.height / 2
                );
                this.ctx.lineTo(
                    (cx + arrowLenX - arrowWidY) * scaleStop + this.canvas.width / 2,
                    -(cy + arrowLenY + arrowWidX) * scaleStop + this.canvas.height / 2
