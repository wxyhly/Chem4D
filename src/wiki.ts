...
酢::(Dione)基团-CO2，泎酸酐。结构式：$$X-C(=O)2$$。此外有以下共振式：$$X-C(<<#O)->>O$$、$$X-C(->>O)<<#O$$。
砏::基团RFnFn。结构式：$$R(<=Fn)(->Fn)$$，默认R为碳：-CFnFn-。
枌::(Funtone)基团X<=Fn。
泎酸::(Carboxylate acid)基团-COOOD2，结构式：$$X-C(=O)(OD)2$$，一种二元氘酸，容易脱水形成酢。
羧酸::(Carboxylic acid)基团-COOD-，结构式：$$X-C(=O)(OD)-X$$，一种一元氘酸。
...
氠::(Shania)ED4，即硝氠，亦可指其它元素的一个原子与四个氢原子形成的化合物。
...
羏::NyT2一价正离子，类似于铵。
钟::(Shanum)ED5二价正离子，类似于铵。
镫::(Tenarium)TnD5三价正离子，类似于铵。
铵::(Ammonium)ND4二价正离子。
...
`.split(/[\t\n%\#\@]/).filter(e => e).map((v, id) => [v.slice(0, -1).replace(/[\u4e00-\uFfa5]/g, ""), v.replace(/[A-z]/g, ""), "("+englishName[v.slice(0, -1).replace(/[\u4e00-\uFfa5]/g, "")]+ (lang==="zh"?
")原子序数：" + (id + 1) + "，价电子排布：" + electrons[id] + "。":
")Atom number: " + (id + 1) + "，Valent electrons: " + electrons[id] + "。")]);
...
export const writeWikiElementsFromAtoms = (dom: HTMLDivElement, atoms: AtomNode[]) => {
...
export const writeWiki = (dom: HTMLDivElement, key: string, value?: string) => {
    value ??= wikiData[key];
    if (value) {
